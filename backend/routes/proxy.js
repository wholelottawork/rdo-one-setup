...
export default async function proxyRoutes(fastify) {
  ...
  // `Vary: Origin` and no CORS grant) get silently rejected by Chrome's
  // Opaque Response Blocking when hotlinked from an <img> tag cross-origin —
  // the image fetches fine server-side (confirmed directly), it's purely a
  // browser-side block. Proxying through our own origin sidesteps it, same
  // fix as the RSS feeds above.
  const PRIVATE_HOST_RE = /^(localhost|127\.|0\.0\.0\.0|10\.|172\.(1[6-9]|2\d|3[01])\.|192\.168\.|169\.254\.|\[?::1\]?$|f[cd][0-9a-f]{0,2}:)/i;

  fastify.get("/img-proxy", async (req, reply) => {
