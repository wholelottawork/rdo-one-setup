...
import {
  ...
  type RawFill, type CumPoint, type ShareData,
} from './pnl';
...
interface EIP1193 { request: (a: { method: string; params?: unknown[] }) => Promise<unknown>; isPhantom?: boolean }
...
export default function PortfolioPage() {
  ...
  const [evmBal, setEvmBal] = useState<ArbBalances | 'loading' | 'error' | null>(null);
  const [evmHintMsg, setEvmHintMsg] = useState<string | null>(null);
  const [netBtn, setNetBtn] = useState<'idle' | 'added' | 'failed' | 'nowallet'>('idle');
  ...
  const [perps, setPerps] = useState<PerpsState | 'loading' | 'error' | null>(null);
  ...
  const loadHLData = useCallback(async (address: string) => {
    ...
    try {
      ...
      const res = await fetch(HL_API, {
        ...
        body: JSON.stringify({ type: 'userFillsByTime', user: address, startTime, endTime: Date.now() }),
      });
  ...
  async function loadPerpsPortfolio(address: string) {
    ...
    try {
      ...
      const positions = ((data.assetPositions || []) as Array<{ position: { coin: string; szi: string; unrealizedPnl?: string } }>)
  ...
  const setEVMAddress = useCallback((addr: string, source: string) => {
    setEvmAddr(addr);
    setEvmSource(source);
    setHlAddrInput(addr);
    loadHLData(addr);
    setEvmBal('loading');
    loadArbitrumBalances(addr).then(setEvmBal).catch(() => setEvmBal('error'));
  }, [loadHLData]);
  ...
  const autoDetectEVM = useCallback(async () => {
    ...
    if (phEvm) {
      try {
        let accs = (await phEvm.request({ method: 'eth_accounts' })) as string[];
        if (!accs?.[0]) accs = (await phEvm.request({ method: 'eth_requestAccounts' })) as string[];
        if (accs?.[0]) { setEVMAddress(accs[0], 'Phantom'); return; }
      } catch { /* EVM not configured in Phantom — fall through */ }
    }
    const prov = w.ethereum;
    if (prov && !prov.isPhantom) {
      try {
        const accs = (await prov.request({ method: 'eth_accounts' })) as string[];
        if (accs?.[0]) { setEVMAddress(accs[0], 'Wallet'); return; }
      } catch { /* silent */ }
    }
  }, [setEVMAddress]);
  ...
  async function connectEVM() {
    ...
    if (phEvm) {
      try {
        ...
        if ((e as { code?: number }).code !== 4001) evmHint('Enable EVM in Phantom → Settings → Networks, then try again.');
        return;
      }
    }
    ...
    try {
      ...
      if ((e as { code?: number }).code !== 4001) evmHint('Connection failed — enter your Hyperliquid address manually.');
    }
  }
  ...
  async function connectWallet() {
    ...
    if (!p?.isPhantom) { setInstallHint(true); return; }
    setConnecting(true);
    try {
  ...
  async function disconnectWallet() {
    try { await getPhantomSolana()?.disconnect(); } catch { /* ignore */ }
    setPubkey(null);
    setAssets(null);
    clearEVMAddr();
  }
  ...
  useEffect(() => {
    ...
    const p = getPhantomSolana();
    if (!p) { setInstallHint(true); return; }
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === 'Escape') { setDepOpen(false); setSwapOpen(false); setConvertOpen(false); setCalOpen(false); }
    };
  ...
  const hl = useMemo(() => {
    ...
    if (closing.length > 1) {
      ...
      Object.values(byCoins).forEach(coinFills => {
        ...
        for (let i = 1; i < cs.length; i++) {
          ...
          if (d > 0 && d < 30 * 86400000) { totalHold += d; holdCount++; }
        }
      });
    ...
    let cum = 0;
    const cumPts: CumPoint[] = sorted.map(f => { cum += parseFloat(f.closedPnl!); return { t: f.time, v: cum }; });
    ...
    return {
      noPeriodData, closing, totalPnl, wins, losses, winRate, bestVal, worstVal, bestFill, worstFill,
      avgHoldMs, cumPts, dailyPnl, recent, totalFills: fills.length, lastTradeTime: sorted.length ? sorted[sorted.length - 1].time : null,
    };
  }, [hlFills, range]);
  ...
  useEffect(() => {
    if (hl && loadedAddr) {
      shareDataRef.current = {
        totalPnl: hl.totalPnl, winRate: hl.winRate, wins: hl.wins, losses: hl.losses,
        bestVal: hl.bestVal, cumPts: hl.cumPts, address: loadedAddr,
      };
    }
  }, [hl, loadedAddr]);

  const hlChart = useMemo(() => buildPnLChartSvg(hl?.noPeriodData ? [] : (hl?.cumPts ?? []), hl?.totalPnl), [hl]);
  const hlDist = useMemo(() => buildDistributionHtml(hl && !hl.noPeriodData ? hl.closing : []), [hl]);
  ...
  function depMax() {
    if (!depToken) return;
    setDepAmount(depToken.balance > 0 ? String(Math.floor(depToken.balance * 1e6) / 1e6) : '');
  }
  ...
  function openPerpsDeposit() {
    ...
    setDepFrameSrc('/swap?mode=deposit' + (addr ? '&toAddress=' + encodeURIComponent(addr) : ''));
  }
  ...
  const evmTotal = evmBal && evmBal !== 'loading' && evmBal !== 'error' ? evmBal.eth * evmBal.ethPriceUsd + evmBal.usdc : 0;
  ...
  return (
    <>
      ...
      <main>
        {/* ══ WALLET SECTION ══ */}
        <div id="connect-screen" style={pubkey ? { display: 'none' } : undefined}>
          ...
          <button className="connect-btn" id="connect-btn" disabled={connecting} onClick={connectWallet}>
            <svg width="16" height="16" viewBox="0 0 128 128" fill="none"><path d="M110.584 64.9142H99.142C99.142 41.8864 80.6366 23.0625 57.9584 23.0625C35.5556 23.0625 17.2065 41.508 16.8677 64.0735C16.5221 87.0972 35.3756 106 58.2219 106H63.3743C85.6702 106 116.581 88.3047 116.581 66.8896C116.581 65.6864 115.684 64.9142 110.584 64.9142Z" fill="white" /></svg>
            {connecting ? 'Connecting…' : ' Connect Phantom'}
          </button>
          <div className="install-hint" id="install-hint" style={installHint ? { display: 'block' } : undefined}>
            Phantom not detected — <a href="https://phantom.app" target="_blank" rel="noopener noreferrer">install Phantom</a> then refresh.
          </div>
        </div>

        <div id="portfolio-screen" style={pubkey ? { display: 'block' } : undefined}>
          <div className="pf-topbar">
            <div className="addr-row">
              <div className="addr-chip" id="addr-chip" onClick={copyFullAddr} title="Click to copy">
                {chipCopied ? '✓ Copied' : pubkey ? shorten(pubkey) : '—'}
              </div>
              <button className="disc-btn" onClick={disconnectWallet}>{t('disconnect')}</button>
            </div>
            <button className="refresh-btn" onClick={() => pubkey && loadPortfolio(pubkey)}>↺ Refresh</button>
          </div>
          <div className="total-card">
            <div>
              <div className="total-lbl">{t('totalPortfolio')}</div>
              <div className="total-val" id="total-val">{assets ? '$' + fmt(totalVal) : '$—'}</div>
              <div className="total-sub" id="total-sub">
                {assetsError ? 'Error' : assets ? `${assets.length} asset${assets.length !== 1 ? 's' : ''}` : 'Loading…'}
              </div>
            </div>
            <div className="action-bar">
              ...
              <button className="act-btn outline" onClick={() => { setSwapOpen(true); if (!swapSrc) setSwapSrc('/swap?mode=swap'); }}>
              ...
              <button className="act-btn outline" onClick={() => { setConvertOpen(true); if (!convertSrc) setConvertSrc('/swap?mode=convert'); }}>
                  ...
                  <div className="asset-row" key={a.mint}>
                    <div className="asset-left">
                      {a.logo
                        ...
                        ? <img className="token-icon" src={a.logo} alt="" onError={e => { (e.target as HTMLImageElement).outerHTML = `<div class=token-icon-ph>${(a.symbol || '?')[0]}</div>`; }} />
                        : <div className="token-icon-ph">{(a.symbol || '?')[0]}</div>}
                      <div>
                        <div className="token-name">{a.name}</div>
                        <div className="token-sym">{a.symbol}</div>
                      </div>
                    </div>
                    <div className="cell">{a.price ? fmtUSD(a.price) : '—'}</div>
                    <div className="cell">{fmt(a.balance, a.balance < 1 ? 4 : 2)}</div>
                    <div className="cell val">{a.value > 0.005 ? '$' + fmt(a.value) : '—'}</div>
                  </div>
                ))
              )}
            </div>
          ...
          <div className={`evm-bal-card${evmAddr ? ' visible' : ''}`} id="evm-bal-card">
            <div className="evm-bal-hdr">
              EVM Wallet
              <span className="evm-bal-chain" id="evm-bal-chain-tag">Arbitrum</span>
            </div>
            <div id="evm-bal-body">
              <div className="evm-bal-row">
                ...
                <div className="evm-bal-right">
                  <div className="evm-bal-amount" id="evm-eth-bal">{evmBal === 'loading' ? '…' : evmBal && evmBal !== 'error' ? fmt(evmBal.eth, evmBal.eth < 0.01 ? 4 : 3) + ' ETH' : '—'}</div>
                  <div className="evm-bal-usd" id="evm-eth-usd">{evmBal && evmBal !== 'loading' && evmBal !== 'error' && evmBal.ethPriceUsd ? '$' + fmt(evmBal.eth * evmBal.ethPriceUsd) : ''}</div>
                </div>
              </div>
              <div className="evm-bal-row">
                ...
                <div className="evm-bal-right">
                  <div className="evm-bal-amount" id="evm-usdc-bal">{evmBal === 'loading' ? '…' : evmBal && evmBal !== 'error' ? fmt(evmBal.usdc) + ' USDC' : '—'}</div>
                  <div className="evm-bal-usd" id="evm-usdc-usd">{evmBal && evmBal !== 'loading' && evmBal !== 'error' ? (evmBal.usdc > 0 ? '$' + fmt(evmBal.usdc) : '—') : ''}</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="section-divider" style={{ marginTop: 32 }}><span>Trader PnL</span></div>
        <div className="pnl-header">
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div className="pnl-title" id="pf-section-title" style={pfMode === 'aster' ? { color: '#f59e0b' } : undefined}>
                {pfMode === 'hl' ? t('traderPnl') : 'ASTER PNL'}
              </div>
              <div className="mode-switch" id="pfModeSwitch">
                <button className={`mode-btn mode-hl${pfMode === 'hl' ? ' active' : ''}`} id="pfBtnHL" onClick={() => switchPortfolioMode('hl')}>BASIC</button>
                <button className={`mode-btn mode-aster${pfMode === 'aster' ? ' active' : ''}`} id="pfBtnAster" onClick={() => switchPortfolioMode('aster')}>EXTRA</button>
              </div>
            </div>
            <div className="pnl-addr-sub" id="pnl-addr-sub">{loadedAddr ? loadedAddr.slice(0, 10) + '…' + loadedAddr.slice(-6) : ''}</div>
          </div>
          <div className="pnl-controls" id="pf-hl-controls">
            <button className="pnl-cal-btn" onClick={() => setCalOpen(true)}>{t('pnlCalendar')}</button>
        ...
        <div id="pnl-section-hl" style={pfMode === 'hl' ? undefined : { display: 'none' }}>
          <div className="hl-connect-bar">
            <input
              className="hl-addr-input"
              id="hl-addr-input"
              placeholder={evmHintMsg ?? 'Enter Hyperliquid wallet address (0x…)'}
              style={evmHintMsg ? { borderColor: 'var(--red)' } : undefined}
              value={hlAddrInput}
              onChange={e => setHlAddrInput(e.target.value)}
            />
            <button className="hl-load-btn" onClick={() => { const a = hlAddrInput.trim(); if (a) loadHLData(a); }}>Load</button>
            {evmAddr ? (
              <button className="hl-evm-btn" id="hl-evm-btn" style={{ borderColor: 'var(--green)', color: 'var(--green)' }} title="Click to disconnect" onClick={clearEVMAddr}>
                <svg width="8" height="8" viewBox="0 0 8 8"><circle cx="4" cy="4" r="4" fill="#1fa67d" /></svg> {evmSource}: {shorten(evmAddr)}
              </button>
            ) : (
              <button className="hl-evm-btn" id="hl-evm-btn" onClick={connectEVM}>
            ...
            <button
              ...
              style={netBtn === 'added' ? { borderColor: 'var(--green)', color: 'var(--green)' } : netBtn === 'failed' || netBtn === 'nowallet' ? { color: 'var(--red)' } : undefined}
              onClick={addHyperEVM}
            >
              {netBtn === 'added' ? (
                <><svg width="8" height="8" viewBox="0 0 8 8"><circle cx="4" cy="4" r="4" fill="#1fa67d" /></svg> HyperEVM Added</>
              ) : netBtn === 'nowallet' ? 'No wallet found' : netBtn === 'failed' ? 'Failed' : (
                <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><path d="M12 8v8M8 12h8" /></svg> Add HyperEVM</>
              )}
            </button>
          </div>
          ...
          <div className="pnl-layout">
            <div className="pnl-main">
              ...
              <div className="pnl-stats">
                <div className="stat-card"><div className="stat-lbl">{t('totalRealizedPnl')}</div><div className={`stat-val ${hl && !hl.noPeriodData ? pCls(hl.totalPnl) : ''}`.trim()} id="s-total-pnl">{hlLoading ? '…' : hl && !hl.noPeriodData ? (hl.totalPnl >= 0 ? '+' : '') + '$' + fmt(hl.totalPnl) : '—'}</div><div className="stat-sub" id="s-total-pnl-sub">All-time</div></div>
                <div className="stat-card"><div className="stat-lbl">{t('winRate')}</div><div className="stat-val" id="s-win-rate">{hlLoading ? '…' : hl && !hl.noPeriodData ? hl.winRate : '—'}</div><div className="stat-sub" id="s-win-sub">{hl && !hl.noPeriodData ? `${hl.wins}W / ${hl.losses}L` : 'wins / losses'}</div></div>
                <div className="stat-card"><div className="stat-lbl">{t('totalTrades')}</div><div className="stat-val" id="s-trades">{hlLoading ? '…' : hl && !hl.noPeriodData ? hl.closing.length.toLocaleString() : '—'}</div><div className="stat-sub" id="s-trades-sub">{hl && !hl.noPeriodData ? hl.totalFills.toLocaleString() + ' total fills' : 'closed positions'}</div></div>
                <div className="stat-card"><div className="stat-lbl">{t('avgHoldTime')}</div><div className="stat-val" id="s-hold">{hlLoading ? '…' : hl && !hl.noPeriodData && hl.avgHoldMs > 0 ? formatDuration(hl.avgHoldMs) : '—'}</div><div className="stat-sub">per trade</div></div>
                <div className="stat-card"><div className="stat-lbl">{t('bestTrade')}</div><div className="stat-val pos" id="s-best">{hlLoading ? '…' : hl && !hl.noPeriodData && hl.bestVal ? '+$' + fmt(hl.bestVal) : '—'}</div><div className="stat-sub" id="s-best-sub">{hl?.bestFill ? hl.bestFill.coin + ' · ' + fmtDate(hl.bestFill.time) : ''}</div></div>
                <div className="stat-card"><div className="stat-lbl">{t('worstTrade')}</div><div className="stat-val neg" id="s-worst">{hlLoading ? '…' : hl && !hl.noPeriodData && hl.worstVal ? '-$' + fmt(Math.abs(hl.worstVal)) : '—'}</div><div className="stat-sub" id="s-worst-sub">{hl?.worstFill ? hl.worstFill.coin + ' · ' + fmtDate(hl.worstFill.time) : ''}</div></div>
              </div>
              ...
              <div className="charts-row">
                <div className="chart-card cum">
                  <div className="chart-hdr">
                    <div className="chart-title">Cumulative PnL · <span id="range-label">{range}</span></div>
                    <div className={hlChart.totalCls} id="chart-total">{hlChart.total}</div>
                  </div>
                  <svg id="pnl-chart-svg" viewBox="0 0 800 200" preserveAspectRatio="none"
                    dangerouslySetInnerHTML={{ __html: hlFills ? hlChart.svg : '<text x="400" y="105" text-anchor="middle" font-size="11" fill="#4a5568">Load a wallet to see PnL chart</text>' }} />
                </div>
                <div className="chart-card dist">
                  <div className="chart-hdr"><div className="chart-title">PnL Distribution</div></div>
                  <div className="dist-bars" id="dist-bars" dangerouslySetInnerHTML={{ __html: hlLoading ? '<div style="color:var(--text3);font-size:11px;text-align:center;padding:40px 0">Loading…</div>' : hlDist }} />
                </div>
              </div>
              ...
              <div className="trades-card">
                <div className="trades-title">
                  ...
                  <span id="trades-count" style={{ fontSize: 11, fontWeight: 500, color: 'var(--text3)' }}>
                    {hl && !hl.noPeriodData ? hl.recent.length + ' of ' + hl.closing.length + ' trades' : ''}
                  </span>
                </div>
                <div id="trades-body">
                  {hlLoading ? (
                    ...
                    <div className="hl-placeholder">Enter your Hyperliquid address above to view trading history.</div>
                  ) : hl?.noPeriodData ? (
                    <div className="hl-placeholder">No trades in this period.</div>
                  ) : !hlFills.length ? (
                    <div className="hl-placeholder">No trades found for this address.</div>
                  ) : !hl?.recent.length ? (
                    <div className="hl-placeholder">No closed trades found.</div>
                  ) : (
                    <>
                      <div className="trades-grid trades-hdr"><span>Token</span><span>Side</span><span>Entry</span><span>Exit</span><span>Size</span><span>PnL</span><span>PnL%</span><span>Date</span></div>
                      {hl.recent.map((f, i) => {
                        ...
                        return (
                          <div className="trades-grid trade-row" key={i}>
                            ...
                            <div className="trade-cell">${fmt(entryPx, entryPx < 1 ? 4 : 2)}</div>
                            <div className="trade-cell">${fmt(exitPx, exitPx < 1 ? 4 : 2)}</div>
                            <div className="trade-cell">{fmt(sz, sz < 1 ? 4 : 2)}</div>
                            <div className={`trade-pnl ${pCls(pnl)}`}>{pnl >= 0 ? '+' : ''}${fmt(pnl)}</div>
                            <div className={`trade-pnl ${pCls(pnl)}`}>{pnlPct >= 0 ? '+' : ''}{fmt(Math.abs(pnlPct), 1)}%</div>
            ...
            <div className="pnl-sidebar">
              <div className="share-card">
                <div className="share-brand">
                  ...
                  <div className="share-logo">RDO<span>ONE</span></div>
                </div>
                <div id="share-content">
                  {!hl || hl.noPeriodData || !loadedAddr ? (
                    <div className="share-placeholder">Load a Hyperliquid wallet to see your PnL card.</div>
                  ) : (
                    <>
                      <div className="share-addr">{shorten(loadedAddr)}</div>
                      <div className="share-pnl" style={{ color: hl.totalPnl >= 0 ? 'var(--green)' : 'var(--red)' }}>{hl.totalPnl >= 0 ? '+' : ''}${fmt(hl.totalPnl)}</div>
                      <div className="share-pnl-sub">ALL-TIME REALIZED PNL</div>
                      <svg id="share-spark-svg" viewBox="0 0 260 50" preserveAspectRatio="none" style={{ display: 'block', width: '100%', height: 44, margin: '6px 0' }}>
                        <path d={buildSparkPath(hl.cumPts.map(p => p.v), 260, 50)} fill="none" stroke={hl.totalPnl >= 0 ? '#1fa67d' : '#ed7088'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                      </svg>
                      <div className="share-stats">
                        <div><div className="share-stat-lbl">Win Rate</div><div className="share-stat-val">{hl.winRate}</div></div>
                        <div><div className="share-stat-lbl">Trades</div><div className="share-stat-val">{hl.wins + hl.losses}</div></div>
                        <div><div className="share-stat-lbl">Best</div><div className="share-stat-val" style={{ color: 'var(--green)' }}>+${fmt(hl.bestVal)}</div></div>
                      </div>
                      <div className="share-actions">
                        <button className="share-btn primary" onClick={shareCardAction}>{shareCopied ? 'COPIED!' : 'SHARE'}</button>
                        <button className="share-btn secondary" onClick={() => shareDataRef.current && downloadCard(shareDataRef.current)}>DOWNLOAD</button>
                      </div>
                    </>
                  )}
                </div>
              ...
              <div className="perps-val-card" id="perps-val-card">
                <div className="perps-val-hdr">
                  ...
                  <span className="perps-val-live"><span className="share-dot" style={{ width: 5, height: 5 }}></span> LIVE</span>
                </div>
                <div className="perps-val-eq" id="pv-equity" style={perps && perps !== 'loading' && perps !== 'error' && perps.equity < 0 ? { color: 'var(--red)' } : undefined}>
                  {perps === 'loading' ? '…' : perps && perps !== 'error' ? '$' + fmt(perps.equity) : '—'}
                </div>
                <div className="perps-val-sub" id="pv-upnl-row">Unrealized PnL: <span id="pv-upnl" style={perps && perps !== 'loading' && perps !== 'error' ? { color: perps.upnl > 0 ? 'var(--green)' : perps.upnl < 0 ? 'var(--red)' : 'var(--text3)' } : undefined}>
                  {perps === 'loading' ? '…' : perps && perps !== 'error' ? (perps.upnl >= 0 ? '+' : '') + '$' + fmt(perps.upnl) : '—'}
                </span></div>
                <div className="perps-val-rows">
                  <div className="perps-val-row"><span>Position Value</span><span id="pv-ntl">{perps === 'loading' ? '…' : perps && perps !== 'error' ? '$' + fmt(perps.ntl) : '—'}</span></div>
                  <div className="perps-val-row"><span>Available Margin</span><span id="pv-avail">{perps === 'loading' ? '…' : perps && perps !== 'error' ? '$' + fmt(perps.avail) : '—'}</span></div>
                  <div className="perps-val-row"><span>Margin Used</span><span id="pv-margin-used">{perps === 'loading' ? '…' : perps && perps !== 'error' ? '$' + fmt(perps.marginUsed) : '—'}</span></div>
                  <div className="perps-val-row"><span>Account Leverage</span><span id="pv-lev">{perps === 'loading' ? '…' : perps && perps !== 'error' ? perps.lev : '—'}</span></div>
                </div>
                <div id="pv-positions-wrap" style={{ marginTop: 12, display: perps && perps !== 'loading' && perps !== 'error' && perps.positions.length ? 'block' : 'none' }}>
                  <div className="perps-val-pos-hdr">Open Positions</div>
                  <div id="pv-positions">
                    {perps && perps !== 'loading' && perps !== 'error' && perps.positions.slice(0, 6).map((p, i) => (
                      <div className="pv-pos-row" key={i}>
                        <span><span className="pv-pos-coin">{p.coin}</span><span className={`pv-pos-dir ${p.isLong ? 'long' : 'short'}`}>{p.isLong ? 'LONG' : 'SHORT'}</span></span>
                        <span className={`pv-pos-pnl ${pCls(p.upnl)}`}>{p.upnl >= 0 ? '+' : ''}${fmt(p.upnl)}</span>
                      </div>
                    ))}
                  </div>
                </div>
                <div className="perps-val-placeholder" id="pv-placeholder" style={{ display: perps ? 'none' : undefined }}>
                  {perps === 'error' ? 'Failed to load perps data.' : 'Load a Hyperliquid wallet to see perps portfolio.'}
                </div>
        ...
        <div id="pnl-section-aster" style={pfMode === 'aster' ? undefined : { display: 'none' }}>
          ...
          <div className="pnl-layout">
            <div className="pnl-main">
              <div className="pnl-stats">
                <div className="stat-card"><div className="stat-lbl">Total Realized PnL</div><div className={`stat-val ${aster?.closing.length ? pCls(aster.totalPnl) : ''}`.trim()} id="as-total-pnl">{asterLoading ? '…' : aster?.closing.length ? (aster.totalPnl >= 0 ? '+' : '') + '$' + fmt(aster.totalPnl) : '—'}</div><div className="stat-sub">All-time</div></div>
              ...
              <div className="charts-row">
                <div className="chart-card cum">
                  <div className="chart-hdr">
                    <div className="chart-title">Cumulative PnL · <span id="as-range-label">{range}</span></div>
                ...
                <div className="chart-card dist">
                  <div className="chart-hdr"><div className="chart-title">PnL Distribution</div></div>
                  <div className="dist-bars" id="as-dist-bars" dangerouslySetInnerHTML={{ __html: asterDist }} />
                </div>
              </div>
            ...
            <div className="pnl-sidebar">
              <div className="share-card" style={{ borderColor: 'rgba(245,158,11,.2)' }}>
                <div className="share-brand">
                  <div className="share-dot" style={{ background: '#f59e0b' }}></div>
              ...
              <div className="perps-val-card" id="aster-perps-card" style={{ marginTop: 14 }}>
                <div className="perps-val-hdr">
      ...
      {/* ── PnL Calendar Modal ── */}
      <div className={`cal-overlay${calOpen ? ' open' : ''}`} id="cal-overlay" onClick={() => setCalOpen(false)}>
        <div className="cal-modal" onClick={e => e.stopPropagation()}>
          <div className="cal-modal-hdr">
            <div className="cal-modal-title">
              PNL CALENDAR
              <span style={{ padding: '3px 8px', background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 3, fontSize: 10, fontWeight: 600, letterSpacing: '.5px', color: 'var(--text2)' }}>$ USD</span>
            </div>
            <div className="cal-modal-nav">
              <button className="cal-nav-btn" onClick={() => { setCalMonth(m => { if (m - 1 < 0) { setCalYear(y => y - 1); return 11; } return m - 1; }); }}>‹</button>
              <span className="cal-month-lbl" id="cal-month-lbl">{calendar.monthLabel}</span>
              <button className="cal-nav-btn" onClick={() => { setCalMonth(m => { if (m + 1 > 11) { setCalYear(y => y + 1); return 0; } return m + 1; }); }}>›</button>
              <button className="cal-close-btn" onClick={() => setCalOpen(false)}>✕</button>
            </div>
          </div>
          <div className="cal-summary">
            <div className={calendar.monthTotalCls} id="cal-month-total">{calendar.monthTotal}</div>
            <div className="cal-bar-track"><div className="cal-bar-fill" id="cal-bar-fill" style={{ width: calendar.barPct + '%', background: calendar.barColor }}></div></div>
            <div className="cal-winloss">
              <span style={{ color: 'var(--green)' }} id="cal-win-label">{calendar.winLabel}</span>
              <span style={{ color: 'var(--red)' }} id="cal-loss-label">{calendar.lossLabel}</span>
            </div>
          </div>
          <div className="cal-grid" id="cal-grid" dangerouslySetInnerHTML={{ __html: hl ? calendar.gridHtml : '<div class="hl-placeholder" style="grid-column:1/-1">Load a wallet to see calendar</div>' }} />
          <div className="cal-footer">
            <span>Current Streak: <strong id="cal-streak">{calendar.streakLabel}</strong></span>
            <span id="cal-best-streak" dangerouslySetInnerHTML={{ __html: calendar.bestStreakHtml }} />
            <span style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 9, letterSpacing: '.08em' }}>
      ...
      <div className={`overlay${depOpen ? ' open' : ''}`} id="deposit-modal" onClick={() => { setDepOpen(false); depBack(); }}>
        <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 420 }}>
          <div className="modal-hdr">
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <button id="dep-back-btn" onClick={depBack} style={{ display: depStep === 'lifi' ? undefined : 'none', background: 'transparent', border: 'none', color: 'var(--text2)', cursor: 'pointer', fontSize: 20, lineHeight: 1, padding: '0 4px 0 0', fontFamily: 'inherit' }}>‹</button>
              <div className="modal-title" id="dep-modal-title">{depTitle}</div>
            </div>
            <button className="modal-x" onClick={() => { setDepOpen(false); depBack(); }}>×</button>
          </div>
          {/* Step 1: token picker */}
          <div id="dep-step-pick" style={{ padding: '0 16px 16px', display: depStep === 'pick' ? undefined : 'none' }}>
            <div className="xfer-box">
              <div className="xfer-label">
                From Tokens
                <span className="xfer-bal-hint"><span id="dep-token-bal">{depToken ? fmt(depToken.balance, depToken.balance < 1 ? 4 : 2) + ' ' + depToken.symbol : '—'}</span> <button className="xfer-max-btn" onClick={depMax}>Max</button></span>
              </div>
              <div className="xfer-row">
                <button className="xfer-token-btn" id="dep-token-btn" onClick={() => setDepListOpen(o => !o)}>
                  <div id="dep-token-icon-wrap">
                    {depToken?.logo
                      // eslint-disable-next-line @next/next/no-img-element
                      ? <img className="xfer-tok-icon" src={depToken.logo} alt="" />
                      : <div className="xfer-tok-ph">{(depToken?.symbol || 'S')[0]}</div>}
                  </div>
                ...
                <div className="xfer-amount-wrap">
                  <input type="number" id="dep-amount" className="xfer-amount-input" placeholder="0.00" value={depAmount} onChange={e => setDepAmount(e.target.value)} />
                  <div className="xfer-amount-usd" id="dep-amount-usd">${fmt(depUsd)}</div>
                </div>
              </div>
            </div>
            <div id="dep-token-list" className="dep-tok-list" style={{ display: depListOpen ? undefined : 'none' }}>
              {!assets?.length ? (
                <div style={{ padding: 16, color: 'var(--text3)', textAlign: 'center', fontSize: 12 }}>Connect wallet to see tokens</div>
              ) : (
                assets.map((a, i) => (
                  <div className="dep-tok-item" key={i} onClick={() => depSelectToken(a)}>
                    {a.logo
                      // eslint-disable-next-line @next/next/no-img-element
                      ? <img className="dep-tok-item-icon" src={a.logo} alt="" />
                      : <div className="dep-tok-item-ph">{(a.symbol || '?')[0]}</div>}
                    <div className="dep-tok-info"><div className="dep-tok-sym">{a.symbol}</div><div className="dep-tok-name">{a.name}</div></div>
                    <div className="dep-tok-right"><div className="dep-tok-bal">{fmt(a.balance, a.balance < 1 ? 4 : 2)}</div><div className="dep-tok-usd">{a.value > 0.005 ? '$' + fmt(a.value) : '—'}</div></div>
                  </div>
                ))
              )}
            </div>
            ...
            <div className="xfer-box">
              ...
              <div className="xfer-to-row">
                ...
                <div className="xfer-to-right">
                  <div className="xfer-to-val" id="dep-to-val">{fmt(depUsd * 0.995)}</div>
                  <div className="xfer-to-usd" id="dep-to-usd">{depAmtNum ? '~$' + fmt(depUsd * 0.995) : '$0.00'}</div>
                </div>
              </div>
            </div>
            <button className="xfer-go-btn" id="dep-go-btn" onClick={depStartTransfer}>Transfer</button>
          </div>
          {/* Step 2: LI.FI bridge */}
          <div id="dep-step-lifi" style={{ display: depStep === 'lifi' ? undefined : 'none', padding: 0 }}>
            <iframe id="lifi-deposit-frame" className="lifi-frame" title="Bridge via LI.FI" allow="clipboard-write" src={depFrameSrc || undefined}></iframe>
          </div>
        </div>
      </div>

      {/* ── Swap Modal ── */}
      <div className={`overlay${swapOpen ? ' open' : ''}`} id="swap-modal" onClick={() => setSwapOpen(false)}>
        <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 520 }}>
          <div className="modal-hdr"><div className="modal-title">Swap</div><button className="modal-x" onClick={() => setSwapOpen(false)}>×</button></div>
          <div className="modal-body" style={{ padding: 0 }}><iframe id="lifi-swap-frame" className="lifi-frame" title="Swap via LI.FI" allow="clipboard-write" src={swapSrc || undefined}></iframe></div>
        </div>
      </div>

      {/* ── Convert Modal ── */}
      <div className={`overlay${convertOpen ? ' open' : ''}`} id="convert-modal" onClick={() => setConvertOpen(false)}>
        <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 520 }}>
          <div className="modal-hdr"><div className="modal-title">Convert to USDC</div><button className="modal-x" onClick={() => setConvertOpen(false)}>×</button></div>
          <div className="modal-body" style={{ padding: 0 }}><iframe id="lifi-convert-frame" className="lifi-frame" title="Convert via LI.FI" allow="clipboard-write" src={convertSrc || undefined}></iframe></div>
        </div>
      </div>
    </>
  );
